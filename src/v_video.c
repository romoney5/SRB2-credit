...
void V_DrawRotatedPatch(fixed_t x, fixed_t y, fixed_t pscale, fixed_t vscale, angle_t angle, INT32 scrn, patch_t *patch, const UINT8 *colormap, fixed_t sx, fixed_t sy, fixed_t w, fixed_t h)
{
	...
	if (!(scrn & V_NOSCALESTART))
	{
		...
		if (!(scrn & V_SCALEPATCHMASK))
		{
			if (vid.width != BASEVIDWIDTH * dup)
			{
				...
				// so center this imaginary screen
				if (scrn & V_SNAPTORIGHT)
					x += (vid.width - (BASEVIDWIDTH * dup)) * FRACUNIT;
				else if (!(scrn & V_SNAPTOLEFT))
					x += ((vid.width - (BASEVIDWIDTH * dup)) / 2) * FRACUNIT;
				if (perplayershuffle & 4)
					x -= ((vid.width - (BASEVIDWIDTH * dup)) / 4) * FRACUNIT;
				else if (perplayershuffle & 8)
					x += ((vid.width - (BASEVIDWIDTH * dup)) / 4) * FRACUNIT;
			}
			if (vid.height != BASEVIDHEIGHT * dup)
			{
				// same thing here
				if (scrn & V_SNAPTOBOTTOM)
					y += (vid.height - (BASEVIDHEIGHT * dup)) * FRACUNIT;
				else if (!(scrn & V_SNAPTOTOP))
					y += ((vid.height - (BASEVIDHEIGHT * dup)) / 2) * FRACUNIT;
				if (perplayershuffle & 1)
					y -= ((vid.height - (BASEVIDHEIGHT * dup)) / 4) * FRACUNIT;
				else if (perplayershuffle & 2)
					y += ((vid.height - (BASEVIDHEIGHT * dup)) / 4) * FRACUNIT;
			}
		}
	...
	if (vscale != FRACUNIT) // scale height properly
	{
		...
		pheight = FixedMul(pheight, dup);
	}
	else
		pheight = patch->height * dup;
	...
	vector2_t patchstart = {
		-FixedMul(offsetx, dup),
		-FixedMul(offsety, dup)
	}, patchstop;
