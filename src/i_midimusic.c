...
static boolean RegisterSong(void)
{
	...
	*/
    ...
    if (song.file == NULL)
    {
		...
		CONS_Alert(CONS_ERROR, "I_MID_RegisterSong: Failed to load MID.\n");
        return false;
    }
...
boolean I_MID_InitMusic(int device)
{
    ...
    if (!array_size(midi_devices))
    {
		...
		CONS_Alert(CONS_ERROR, "I_MID_InitMusic: No devices found.\n");
        return false;
    }
	...
	CONS_Alert(CONS_ERROR, "MIDI Init: Using '%s'.\n", midi_devices[device]);
...
// TODO: Port these console variables to SRB2's system
