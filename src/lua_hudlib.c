...
static int libd_getSprite2Patch(lua_State *L)
{
	...
	{
		angle_t rollangle = luaL_checkangle(L, 4);
		patch_t *rotsprite = Patch_GetRotatedSprite(sprframe, frame, angle, sprframe->flip & (1<<angle), true, P_GetSkinSpriteInfo(skins[i], j), rollangle);

		// romoney5: and this?
		// if (rot) {
		// 	patch_t *rotsprite = Patch_GetRotatedSprite(sprframe, frame, angle, sprframe->flip & (1<<angle), P_GetSkinSpriteInfo(skins[i], j), rollangle);
			LUA_PushUserdata(L, rotsprite, META_PATCH);
			lua_pushboolean(L, false);
			lua_pushboolean(L, true);
			return 3;
